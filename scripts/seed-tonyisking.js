...
async function seed() {
  ...
  // Check if org already exists — reuse existing orgId to avoid FK violations
  const existingOrg = await db.all("SELECT id FROM organizations WHERE slug = 'tonyisking' OR name = 'Tony is King' LIMIT 1").catch(() => []);
  let orgId;
  if (existingOrg.length > 0) {
    ...
    orgId = uuid();
    await db.run(`INSERT INTO organizations (id,name,slug,plan,feature_flags,status) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (id) DO NOTHING`,
  ...
  const adminId = userIds['company_admin'];
