 ...
 * Data loaded from DB via State.scmInventory + State.scmForecast (centrally loaded in app.js)
 */
import { State, render } from '../../core/state.js';

export function renderPage() {
  // Use State data if available, otherwise fall back to prefetch cache
  const cache = window._opsErpCache || {};
  const inv = State.scmInventory || cache.inventory;
  const fc = State.scmForecast || cache.forecast;
  if (!inv) return '<div class="sa-page"><div class="sa-loading-indicator"><div class="sa-spinner"></div><p>Loading inventory data from database...</p></div></div>';
