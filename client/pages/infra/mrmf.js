...
const fm = s => s ? s.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'N/A';
...
<div class="sa-page">
    <div class="sa-page-title"><h1>${icon('shield')} MRMF v2.0 — Enterprise-Native</h1>
        ...
        <div style="display:inline-flex;gap:6px;flex-wrap:wrap">
            ...
            <span style="padding:3px 10px;border-radius:4px;background:${tc(m?.mhi_score)}22;color:${tc(m?.mhi_score)};font-weight:700;font-size:0.78rem">MHI: ${m?.mhi_score || 'N/A'} (${m?.grade || 'N/A'})</span>
            <span style="padding:3px 10px;border-radius:4px;background:${r?.grade === 'Controlled' ? '#10b98122' : '#f59e0b22'};color:${r?.grade === 'Controlled' ? '#10b981' : '#f59e0b'};font-weight:700;font-size:0.78rem">Residual: ${r?.score || 'N/A'}% (${r?.grade || 'N/A'})</span>
        </div></div>

    <!-- Maturity Ladder -->
    <div class="sa-card" style="margin-bottom:10px"><h3 style="margin:0 0 4px;color:#f1f5f9;font-size:0.82rem">Maturity Ladder</h3>
        <div style="display:flex;gap:4px">${(mt?.levels || []).map(l => `<div style="flex:1;padding:6px;background:${l.status ? '#10b98110' : '#1e293b'};border:1px solid ${l.status ? '#10b981' : '#334155'};border-radius:4px;text-align:center"><div class="infra-badge ${l.status ? 'infra-badge--success' : 'infra-badge--neutral'}">${l.status ? icon('check-circle') : 'N/A'}</div><div style="color:${l.status ? '#10b981' : '#64748b'};font-weight:700;font-size:0.72rem">${l.level}</div><div style="color:#94a3b8;font-size:8px">${l.name}</div></div>`).join('')}</div>
    </div>
    ...
    <div style="overflow-x:auto;margin-bottom:10px"><table style="width:100%;border-collapse:collapse"><thead><tr style="border-bottom:2px solid #334155;color:#94a3b8"><th style="padding:6px;text-align:left">Model</th><th>Class</th><th>Lock</th><th>Auto</th><th>Dual IVU</th><th>Inputs</th><th>Refresh</th><th>Status</th></tr></thead><tbody>${(D.inv?.models || []).map(m => `<tr style="border-bottom:1px solid #1e293b"><td style="padding:4px;color:#3b82f6;font-weight:600">${m.model_type || m.name}</td><td style="text-align:center"><span class="infra-badge ${m.impact_class === 'Critical' ? 'infra-badge--danger' : m.impact_class === 'High' ? 'infra-badge--warning' : m.impact_class === 'Medium' ? 'infra-badge--info' : 'infra-badge--success'}">${m.impact_class}</span></td><td style="text-align:center">${m.lock_capable ? icon('lock') : 'N/A'}</td><td style="text-align:center">${m.auto_decision ? icon('zap') : 'N/A'}</td><td style="text-align:center">${m.dual_validation ? icon('users') : 'N/A'}</td><td style="text-align:center;color:#f1f5f9">${m.inputs}</td><td style="color:#64748b;text-align:center">${m.refresh}</td><td style="text-align:center"><span class="infra-badge infra-badge--warning">${fm(m.validation_status || m.status)}</span></td></tr>`).join('')}</tbody></table></div>
    ...
    <div style="display:grid;grid-template-columns:3fr 2fr;gap:10px;margin-bottom:10px">
        ...
        <div class="sa-card"><h3 style="margin:0 0 4px;color:#f1f5f9;font-size:0.72rem">P3 · Stress Test Library (${D.stressLib?.total || 6})</h3>
            ${(D.stressLib?.tests || []).map(t => `<div style="padding:3px;background:#0f172a;border-radius:3px;margin-bottom:3px;border-left:2px solid ${t.auto_freeze ? '#ef4444' : '#f59e0b'}"><div style="display:flex;justify-content:space-between"><span style="color:#f1f5f9;font-size:8px;font-weight:600">${t.name}</span><span style="font-size:7px;color:${t.auto_freeze ? '#ef4444' : '#f59e0b'}">${t.auto_freeze ? 'AUTO-FREEZE' : 'manual'}</span></div><div style="color:#64748b;font-size:7px">${t.scenario.length > 100 ? t.scenario.slice(0, 100) + '…' : t.scenario}</div><div style="color:#94a3b8;font-size:7px;margin-top:1px">Fallback: ${t.fallback.length > 80 ? t.fallback.slice(0, 80) + '…' : t.fallback}</div></div>`).join('')}
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        ...
        <div class="sa-card"><h3 style="margin:0 0 4px;color:#f1f5f9;font-size:0.72rem">P5 · Model Health Index (MHI)</h3>
            <div style="text-align:center;padding:6px"><div style="font-size:28px;font-weight:800;color:${tc(m?.mhi_score)}">${m?.mhi_score || 'N/A'}</div><div style="color:#94a3b8;font-size:8px">MHI Score (Grade: ${m?.grade || 'N/A'})</div></div>
    ...
    <div class="sa-card" style="margin-top:10px"><h3 style="margin:0 0 4px;color:#f1f5f9;font-size:0.72rem">Gap Map (v1.0 → v2.0)</h3>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:4px">${(mt?.gap_map || []).map(g => `<div style="padding:3px;background:#0f172a;border-radius:3px"><div style="color:#f1f5f9;font-size:8px;font-weight:600">${g.component}</div><div style="color:${g.gap === 'N/A' || g.gap === '—' ? '#10b981' : '#3b82f6'};font-size:7px">${g.current} ${g.gap !== 'N/A' && g.gap !== '—' ? '→ ' + g.gap : ''}</div></div>`).join('')}</div>
    </div>
</div>`;
}
