...
async function load() {
    ...
    setTimeout(() => {
        ...
        if (active && active.getAttribute('data-tab') === 'green-finance') {
...
export function render() {
    load(); const s = D.score; const c = D.collateral; return `
<div class="sa-page">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px">
        <div class="sa-card"><h3 style="margin:0 0 10px;color:#f1f5f9">${icon('barChart')} Green Credit Score Factors</h3>
            ${s?.factors?.map(f => `<div style="display:flex;align-items:center;gap:8px;padding:4px 0"><div style="width:130px;color:#94a3b8;font-size:0.78rem">${f.factor}</div><div style="flex:1;background:#1e293b;border-radius:4px;height:10px;position:relative"><div style="position:absolute;left:0;top:0;height:100%;width:${f.value}%;background:${f.value >= 70 ? '#10b981' : f.value >= 40 ? '#f59e0b' : '#ef4444'};border-radius:4px"></div></div><span style="color:#f1f5f9;font-weight:600;font-size:0.72rem;width:30px;text-align:right">${f.value}</span></div>`).join('') || ''}
        ...
        <div class="sa-card"><h3 style="margin:0 0 10px;color:#f1f5f9">${icon('coins')} Carbon Collateral</h3>
            ${c ? `<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px"><div style="padding:10px;background:#0f172a;border-radius:6px;text-align:center"><div style="color:#10b981;font-weight:700;font-size:16px">${c.total_tCO2e}</div><div style="color:#64748b;font-size:0.68rem">tCO₂e</div></div><div style="padding:10px;background:#0f172a;border-radius:6px;text-align:center"><div style="color:#f59e0b;font-weight:700;font-size:16px">$${c.spot_price_usd}/t</div><div style="color:#64748b;font-size:0.68rem">Spot Price</div></div><div style="padding:10px;background:#0f172a;border-radius:6px;text-align:center"><div style="color:#94a3b8;font-weight:700;font-size:16px">${c.haircut_pct}%</div><div style="color:#64748b;font-size:0.68rem">Haircut</div></div><div style="padding:10px;background:#0f172a;border-radius:6px;text-align:center"><div style="color:#8b5cf6;font-weight:700;font-size:16px">${c.total_credits}</div><div style="color:#64748b;font-size:0.68rem">Credits</div></div></div>` : ''}
        </div>
    </div>
    <div class="sa-card"><h3 style="margin:0 0 10px;color:#f1f5f9">${icon('target')} Green Finance Instruments</h3>
        <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px">${D.instruments?.instruments?.map(i => `<div style="padding:12px;background:#0f172a;border-radius:8px;text-align:center"><div style="color:#f1f5f9;font-weight:600;font-size:0.78rem">${i.name}</div><div style="color:#64748b;font-size:0.68rem;margin-top:4px">${i.description}</div><div style="color:#10b981;font-size:0.68rem;margin-top:4px">Min Score: ${i.min_score}</div></div>`).join('') || ''}</div>
    </div>
</div>`;
}
