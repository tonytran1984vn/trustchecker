...
<div class="sa-page">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
        <div class="sa-card"><h3 style="margin:0 0 6px;color:#f1f5f9;font-size:0.82rem">🚫 Forbidden Actions (NEVER)</h3>
            ${(sa?.forbidden_actions || []).length > 0 ? (sa.forbidden_actions).map(f => `<div style="display:flex;align-items:center;gap:6px;padding:2px 0"><span style="color:#ef4444;font-size:0.82rem"><span class="status-icon status-fail" aria-label="Fail">✗</span></span><span style="color:#f1f5f9;font-size:0.72rem">${f.action}</span></div>`).join('') : '<div style="text-align:center;padding:16px;color:#64748b;font-size:0.78rem">${icon("lock")} Access restricted to Super Admin</div>'}
        </div>
