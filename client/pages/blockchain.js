  ...
  return `
    <div class="stats-grid" style="grid-template-columns:repeat(3,1fr)">
      ...
      <div class="stat-card ${b.stats?.chain_integrity?.valid ? 'emerald' : 'rose'}">
        <div class="stat-icon">${b.stats?.chain_integrity?.valid ? '<span class="status-icon status-pass" aria-label="Pass">✓</span>' : '<span class="status-icon status-fail" aria-label="Fail">✗</span>'}</div>
