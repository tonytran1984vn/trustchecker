...
// ATK-09: Encrypt MFA secrets at rest
const MFA_KEY = process.env.ENCRYPTION_KEY ? Buffer.from(process.env.ENCRYPTION_KEY.padEnd(32, '0').slice(0, 32)) : null;

function encryptSecret(plaintext) {
    if (!MFA_KEY) return plaintext; // Fallback: no encryption if key not set
    const iv = crypto.randomBytes(16);
    const cipher = crypto.createCipheriv('aes-256-cbc', MFA_KEY, iv);
...
function decryptSecret(ciphertext) {
    if (!MFA_KEY || !ciphertext.includes(':')) return ciphertext; // Plain text fallback
    const [ivHex, encrypted] = ciphertext.split(':');
    const iv = Buffer.from(ivHex, 'hex');
    const decipher = crypto.createDecipheriv('aes-256-cbc', MFA_KEY, iv);
...
router.post('/mfa/setup', authMiddleware, async (req, res) => {
    try {
        ...
        await db.prepare('UPDATE users SET mfa_secret = ?, mfa_backup_codes = ? WHERE id = ?')
            .run(encryptSecret(secret), JSON.stringify(hashedCodes), user.id);
...
router.post('/mfa/verify', authMiddleware, async (req, res) => {
    try {
        ...
        if (user && user.mfa_secret) user.mfa_secret = decryptSecret(user.mfa_secret); // ATK-09: decrypt
