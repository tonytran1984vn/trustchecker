...
router.get('/infra-metrics/financial', requireRole('risk_committee'), (req, res) => {
    res.json(metrics.getFinancialMetrics());
});
router.get('/infra-metrics/composite', requireRole('risk_committee'), (req, res) => {
    res.json(metrics.getCompositeScore());
});
router.get('/infra-metrics/governance-surveillance', requireRole('risk_committee'), (req, res) => {
    res.json(metrics.getGovernanceSurveillance());
});
...
router.get('/upgrade-gov/framework', requireRole('risk_committee'), (req, res) => {
    res.json(upgrade.getFullFramework());
});
router.get('/upgrade-gov/classification', requireRole('risk_committee'), (req, res) => {
    res.json(upgrade.getClassification());
});
router.get('/upgrade-gov/cab-process', requireRole('risk_committee'), (req, res) => {
    res.json(upgrade.getCABProcess());
});
router.get('/upgrade-gov/rollback', requireRole('risk_committee'), (req, res) => {
    res.json(upgrade.getRollback());
});
router.get('/upgrade-gov/versioning', requireRole('risk_committee'), (req, res) => {
    res.json(upgrade.getVersionGovernance());
});
...
router.post('/upgrade-gov/classify', requireRole('super_admin'), (req, res) => {
    ...
    res.json(
        upgrade.classifyChange(description, impacts_scoring, impacts_settlement, impacts_schema, impacts_constitution)
    );
});
