...
const identityEngine = require('../engines/infrastructure/identity-engine');
const { v4: uuidv4 } = require('uuid');
const { withTransaction } = require('../middleware/transaction');
router.use(authMiddleware);

// DB schema is natively managed by Prisma (did_registry, verifiable_credentials)
...
router.post('/did', requirePermission('esg:manage'), async (req, res) => {
    try {
        ...
        await db
            .prepare(
                'INSERT INTO did_registry (id,did,entity_type,entity_id,org_id,did_document,public_key) VALUES (?,?,?,?,?,?,?)'
            )
            .run(
                uuidv4(),
                result.did,
                ...
                JSON.stringify(result.did_document),
                result.keys.publicKey
            );
        res.status(201).json(result);
    } catch (err) {
        res.status(500).json({ error: 'DID generation failed' });
    }
});
...
router.get('/did/resolve/:did', async (req, res) => {
    try {
        ...
        res.json(
            identityEngine.resolveDID(row.did, {
                did_document: JSON.parse(row.did_document),
...
router.get('/did/registry', async (req, res) => {
    try {
        const rows = await db.all(
            'SELECT did,entity_type,entity_id,org_id,status,created_at FROM did_registry ORDER BY created_at DESC LIMIT 50'
        );
...
router.post('/vc/issue', requirePermission('esg:manage'), async (req, res) => {
    try {
        ...
        const result = identityEngine.issueVC({
            issuer_did,
            subject_did,
            credential_type,
            claims,
        ...
        await db
            .prepare(
                'INSERT INTO verifiable_credentials (id,vc_id,credential_type,issuer_did,subject_did,credential,proof_hash,valid_until,org_id) VALUES (?,?,?,?,?,?,?,?,?)'
            )
            .run(
                uuidv4(),
                result.vc_id,
                ...
                result.credential.proof.proofValue,
...
router.post('/vc/verify', async (req, res) => {
    try {
        res.json(identityEngine.verifyVC(req.body));
    } catch (err) {
...
router.get('/vc/registry', async (req, res) => {
    try {
        const rows = await db.all(
            'SELECT vc_id,credential_type,issuer_did,subject_did,status,valid_until,created_at FROM verifiable_credentials ORDER BY created_at DESC LIMIT 50'
        );
        res.json({ title: 'Verifiable Credentials Registry', total: rows.length, credentials: rows });
    } catch (err) {
        res.status(500).json({ error: 'VC registry failed' });
    }
});
...
router.get('/trust-chain/:did', async (req, res) => {
    try {
        ...
        const creds = await db.all('SELECT * FROM verifiable_credentials WHERE subject_did = ? AND status = ?', [
            did,
            'active',
        ]);
        const linked = await db.all(
            'SELECT * FROM did_registry WHERE org_id = (SELECT org_id FROM did_registry WHERE did = ?) AND did != ?',
            [did, did]
        );
        ...
        res.status(500).json({ error: 'Trust chain failed' });
    }
});
