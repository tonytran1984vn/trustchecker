...
const init = async () => {
    try {
        await db.exec(`
    CREATE TABLE IF NOT EXISTS api_keys (id TEXT PRIMARY KEY, api_key TEXT UNIQUE NOT NULL, api_secret_hash TEXT, tier TEXT, integration_type TEXT, app_name TEXT, owner_id TEXT, org_id TEXT, status TEXT DEFAULT 'active', rate_limit_rpm INTEGER DEFAULT 100, daily_limit INTEGER DEFAULT 1000, total_requests INTEGER DEFAULT 0, created_at DATETIME DEFAULT (NOW()));
`);
    } catch (e) { }
};
init();
...
router.post('/keys', requirePermission('admin:manage'), async (req, res) => {
    try {
        const result = apiEconomy.generateAPIKey({ ...req.body, org_id: req.user?.org_id, owner_id: req.user?.id });
        if (result.error) return res.status(400).json(result);
        const hash = require('crypto').createHash('sha256').update(result.api_secret).digest('hex');
const { withTransaction } = require('../middleware/transaction');
        await db.prepare('INSERT INTO api_keys (id,api_key,api_secret_hash,tier,integration_type,app_name,owner_id,org_id,rate_limit_rpm,daily_limit) VALUES (?,?,?,?,?,?,?,?,?,?)')
...
router.get('/keys', async (req, res) => {
    try {
        const keys = await db.all('SELECT api_key,tier,integration_type,app_name,status,rate_limit_rpm,daily_limit,total_requests,created_at FROM api_keys WHERE org_id = ?', [req.user?.org_id || req.user?.orgId]);
...
router.get('/usage', async (req, res) => {
    try {
        const keys = await db.all('SELECT * FROM api_keys WHERE org_id = ?', [req.user?.org_id || req.user?.orgId]);
...
router.delete('/keys/:apiKey', requirePermission('admin:manage'), async (req, res) => {
    try {
        await db.run('UPDATE api_keys SET status = ? WHERE api_key = ?', ['revoked', req.params.apiKey]);
