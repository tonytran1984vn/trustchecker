...
router.get('/radar', cacheMiddleware(60), async (req, res) => {
    try {
        ...
        const [partners, shipments, violations, leaks, alerts, inventory, certifications, sustainability] =
            await Promise.all([
                db.prepare('SELECT * FROM partners' + orgFilter).all(...orgParams),
                orgId
                    ? db.all(
                          'SELECT s.* FROM shipments s LEFT JOIN partners fp ON s.from_partner_id = fp.id LEFT JOIN partners tp ON s.to_partner_id = tp.id WHERE fp.org_id = ? OR tp.org_id = ? LIMIT 1000',
                          [orgId, orgId]
                      )
                ...
                orgId
                    ? db.all(
                          'SELECT fa.* FROM fraud_alerts fa LEFT JOIN products p ON fa.product_id = p.id WHERE p.org_id = ? OR p.org_id IS NULL',
                          [orgId]
                      )
                    : db.all('SELECT * FROM fraud_alerts LIMIT 1000'),
                orgId
                    ? db.all(
                          'SELECT i.* FROM inventory i LEFT JOIN products p ON i.product_id = p.id WHERE p.org_id = ? OR p.org_id IS NULL',
                          [orgId]
                      )
...
router.get('/alerts', async (req, res) => {
    try {
        ...
        const [fraudAlerts, leakAlerts, slaAlerts, anomalyAlerts] = await Promise.all([
            db
                .all(
                    `SELECT fa.id, 'fraud' as source, fa.alert_type, fa.severity, fa.description, fa.status, fa.created_at FROM fraud_alerts fa LEFT JOIN products p ON fa.product_id = p.id WHERE fa.status = 'open'${_safeJoin(orgJoin)} ORDER BY fa.created_at DESC LIMIT ?`,
                    [...orgP, cappedLimit]
                )
                .catch(() => []),
            db
                .prepare(
                    "SELECT id, 'leak' as source, leak_type as alert_type, CASE WHEN risk_score > 0.7 THEN 'high' WHEN risk_score > 0.4 THEN 'medium' ELSE 'low' END as severity, listing_title as description, status, created_at FROM leak_alerts WHERE status = 'open' ORDER BY created_at DESC LIMIT ?"
                )
                .all(cappedLimit)
                .catch(() => []),
            db
                .prepare(
                    "SELECT id, 'sla' as source, violation_type as alert_type, CASE WHEN penalty_amount > 1000 THEN 'high' WHEN penalty_amount > 100 THEN 'medium' ELSE 'low' END as severity, violation_type as description, status, created_at FROM sla_violations WHERE status = 'open' ORDER BY created_at DESC LIMIT ?"
                )
                .all(cappedLimit)
                .catch(() => []),
            db
                .prepare(
                    "SELECT id, 'anomaly' as source, anomaly_type as alert_type, severity, description, status, detected_at as created_at FROM anomaly_detections WHERE status = 'open' ORDER BY detected_at DESC LIMIT ?"
                )
                .all(cappedLimit)
                .catch(() => []),
        ]);

        let allAlerts = [...fraudAlerts, ...leakAlerts, ...slaAlerts, ...anomalyAlerts].sort(
...
router.get('/trends', async (req, res) => {
    try {
        ...
        const [fraudTrend, leakTrend, violationTrend] = await Promise.all([
            db.all(
                `SELECT DATE(fa.created_at) as day, COUNT(*)::int as count FROM fraud_alerts fa LEFT JOIN products p ON fa.product_id = p.id WHERE fa.created_at >= ?${_safeJoin(orgJoin)} GROUP BY DATE(fa.created_at) ORDER BY day LIMIT 1000`,
                [since, ...orgP]
            ),
            orgId
                ? db.all(
                      'SELECT DATE(la.created_at) as day, COUNT(*)::int as count FROM leak_alerts la LEFT JOIN products p ON la.product_id = p.id WHERE la.created_at >= ? AND p.org_id = ? GROUP BY DATE(la.created_at) ORDER BY day LIMIT 1000',
                      [since, orgId]
                  )
                : db.all(
                      'SELECT DATE(created_at) as day, COUNT(*)::int as count FROM leak_alerts WHERE created_at >= ? GROUP BY DATE(created_at) ORDER BY day LIMIT 1000',
                      [since]
                  ),
            orgId
                ? db.all(
                      'SELECT DATE(sv.created_at) as day, COUNT(*)::int as count FROM sla_violations sv LEFT JOIN partners p ON sv.partner_id = p.id WHERE sv.created_at >= ? AND p.org_id = ? GROUP BY DATE(sv.created_at) ORDER BY day LIMIT 1000',
                      [since, orgId]
                  )
                : db.all(
                      'SELECT DATE(created_at) as day, COUNT(*)::int as count FROM sla_violations WHERE created_at >= ? GROUP BY DATE(created_at) ORDER BY day LIMIT 1000',
                      [since]
                  ),
        ]);
