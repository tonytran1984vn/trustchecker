...
router.get('/report', cacheMiddleware(120), async (req, res) => {
    try {
        ...
        const report = compliance.generateComplianceReport({
            ...
            scope_3: 420,
            total_emissions: 650,
            products_count: products.cnt,
            partners_count: partners.cnt,
...
router.get('/diff', async (req, res) => {
    try {
        ...
        const current = compliance.generateComplianceReport({
            scope_1: 150,
            scope_2: 80,
            scope_3: 420,
            total_emissions: 650,
        ...
        const baseline = compliance.generateComplianceReport({
            scope_1: 100,
            scope_2: null,
            scope_3: null,
            total_emissions: 100,
        ...
        res.status(500).json({ error: 'Diff tracking failed' });
    }
});
...
router.get('/gaps', cacheMiddleware(120), async (req, res) => {
    try {
        ...
        res.json({
            ...
            gaps: report.gaps,
            overall_readiness: report.overall_readiness_pct,
        });
    } catch (err) {
...
router.get('/cbam-status', (req, res) => {
    ...
    res.json({
        ...
        cbam_affected: fw.cbam_affected,
        region,
        export_destinations: exports,
        applicable_frameworks: fw.applicable_frameworks.length,
