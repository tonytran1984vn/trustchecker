    ...
    try {
        ...
        var result = await db.all(
            ...
            [
                ...
                b.client_secret,
                b.metadata_url,
                JSON.stringify(b.attribute_mapping || {}),
                b.enabled || false,
                b.enforce || false,
            ]
        );
    ...
    try {
        ...
        if (config.provider === 'saml') {
            ...
            var samlRequest = Buffer.from(
                '<samlp:AuthnRequest xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" ID="_' +
                    ...
                    '/api/sso/callback"><saml:Issuer xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">' +
                    (config.issuer_url || 'trustchecker') +
                    '</saml:Issuer></samlp:AuthnRequest>'
            ).toString('base64');
            return res.redirect(
                config.sso_url + '?SAMLRequest=' + encodeURIComponent(samlRequest) + '&RelayState=' + state
            );
        }
        ...
        if (config.provider === 'oauth2' || config.provider === 'oidc') {
            var authUrl =
                ...
                encodeURIComponent((process.env.BASE_URL || 'http://localhost:4000') + '/api/sso/callback') +
...
router.get('/metadata/:orgSlug', async function (req, res) {
    ...
    var metadata =
        ...
        '/api/sso/metadata/' +
        req.params.orgSlug +
        '"><md:SPSSODescriptor protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol"><md:AssertionConsumerService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="' +
        baseUrl +
        '/api/sso/callback" index="0" isDefault="true"/></md:SPSSODescriptor></md:EntityDescriptor>';
