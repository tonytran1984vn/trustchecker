...
function logConstitutionalAction(req, action, result) {
    ...
    try {
        ...
        db.prepare(
            ...
            action,
            JSON.stringify({ allowed: entry.allowed, reason: entry.reason, charter: entry.charter }),
            entry.ip,
            entry.timestamp
        );
    } catch (e) {
    ...
    return (req, res, next) => {
        ...
        if (!secondApprover || !secondRole) {
            logConstitutionalAction(req, action, {
                allowed: false,
                reason: `Multi-party required: ${requiredRoles.join(' + ')}`,
            });
            return res.status(403).json({
        ...
        logConstitutionalAction(req, `${action}:multi_party_passed`, {
            allowed: true,
            reason: `${req.user.role} + ${secondRole}`,
        });
        req._multi_party = {
            first: req.user.id,
...
router.get('/topology', (req, res) => {
    res.json(network.getTopology());
});
router.get('/health', (req, res) => {
    res.json(network.getNetworkHealth());
});
...
router.get('/config', (req, res) => {
    res.json({
        node_types: network.getNodeTypes(),
        regions: network.getRegions(),
        consensus: network.getConsensusParams(),
    });
});
...
router.post('/nodes/:id/activate', requireConstitutionalWithAudit('network.validator.admit'), (req, res) => {
...
router.post('/consensus', requireConstitutionalWithAudit('network.consensus.view'), (req, res) => {
