    ...
    `,
        ...
        await db
            ...
            .run(
                ...
                'ticket',
                id,
                JSON.stringify({ subject, category, priority })
            );
...
router.get('/', async (req, res) => {
    try {
        ...
        let sql =
            'SELECT st.*, u.username as user_name FROM support_tickets st LEFT JOIN users u ON st.user_id = u.id WHERE 1=1';
        const params = [];
        ...
        sql +=
            " ORDER BY CASE st.priority WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 ELSE 4 END, st.created_at DESC LIMIT ?";
        ...
        const stats = {
            ...
            in_progress:
                (await db.get("SELECT COUNT(*) as c FROM support_tickets WHERE status = 'in_progress'"))?.c || 0,
...
router.get('/stats/summary', requirePermission('org:user_create'), async (req, res) => {
    try {
        ...
        const avgResolution = await db.get(
            'SELECT AVG(JULIANDAY(resolved_at) - JULIANDAY(created_at)) * 24 as avg_hours FROM support_tickets WHERE resolved_at IS NOT NULL'
        );
...
router.get('/:id', async (req, res) => {
    try {
        const ticket = await db.get(
            'SELECT st.*, u.username as user_name FROM support_tickets st LEFT JOIN users u ON st.user_id = u.id WHERE st.id = ?',
            [req.params.id]
        );
...
router.post('/:id/message', async (req, res) => {
    try {
        ...
        if (['resolved', 'closed'].includes(ticket.status) && ticket.user_id === req.user.id) {
            await db
                .prepare("UPDATE support_tickets SET status = 'open', updated_at = NOW() WHERE id = ?")
                .run(req.params.id);
        } else {
    ...
    try {
        ...
        await db
            ...
            .run(
                ...
                'ticket',
                req.params.id,
                JSON.stringify({ assigned_to, assignee_name: assignee.username })
            );
...
router.put('/:id/resolve', async (req, res) => {
    try {
        ...
        await db
            .prepare(
                "UPDATE support_tickets SET status = 'resolved', resolution = ?, resolved_at = NOW(), updated_at = NOW() WHERE id = ?"
            )
...
router.put('/:id/close', async (req, res) => {
    try {
        ...
        await db
            .prepare("UPDATE support_tickets SET status = 'closed', updated_at = NOW() WHERE id = ?")
            .run(req.params.id);
