 ...
 *
 * Manages roles, users, and permissions within a single org.
 * All routes are scoped to req.orgId via orgGuard.
 *
 * Base path: /api/org
 */
...
// ─── GET /roles — List roles for this org ─────────────────────────────────
router.get('/roles', async (req, res) => {
    try {
       ...
       FROM rbac_roles r 
       WHERE r.org_id = ?
       ORDER BY r.is_system DESC, r.name LIMIT 1000`,
            [req.orgId]
        );
...
router.post('/roles', async (req, res) => {
    try {
        ...
        const roleId = uuidv4();
        await db.run(
            `INSERT INTO rbac_roles (id, org_id, name, display_name, type, is_system, description) VALUES (?, ?, ?, ?, 'org', 0, ?)`,
            [roleId, req.orgId, name, display_name, description]
        );
        ...
        await db.run(
            ...
            [uuidv4(), req.user.id, roleId, JSON.stringify({ name, permissions: mapped, org_id: req.orgId })]
        );
...
router.get('/permissions', async (req, res) => {
    try {
        // Company Admin can see org + business permissions (NOT platform)
        const permissions = await db.all(
            `SELECT id, resource, action, scope, level, description 
       FROM rbac_permissions 
       WHERE level IN ('org', 'business')
       ORDER BY level, resource, action LIMIT 1000`
        );
...
// ─── POST /users — Create user within org ─────────────────────────────────
router.post('/users', async (req, res) => {
    try {
        ...
        await db.run(
            ...
            [userId, username, email, hash, role, companyName, req.orgId]
        );

        await db.run(
            `INSERT INTO audit_log (id, actor_id, action, entity_type, entity_id, details) VALUES (?, ?, 'USER_CREATED', 'user', ?, ?)`,
            [uuidv4(), req.user.id, userId, JSON.stringify({ username, role, org_id: req.orgId })]
        );
    ...
    try {
        ...
        for (const roleId of role_ids) {
            ...
            if (
                ...
                await db.run(
                    `INSERT INTO pending_role_approvals (id, user_id, role_id, role_name, requested_by, org_id, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [approvalId, req.params.id, roleId, roleInfo.name, req.user.id, req.orgId, expires_at]
                );
        ...
        const orgRoles = await db.all('SELECT id FROM rbac_roles WHERE org_id = ?', [req.orgId]);
        const orgRoleIds = orgRoles.map(r => r.id);
        for (const rid of orgRoleIds) {
        ...
        for (const roleId of role_ids) {
            await db.run(
                'INSERT INTO rbac_user_roles (user_id, role_id, assigned_by, expires_at) VALUES (?, ?, ?, ?) ON CONFLICT DO NOTHING',
...
router.delete('/users/:id', async (req, res) => {
    try {
        ...
        // Remove from org (set org_id to NULL, don't delete user row)
        ...
        await db.run(
            ...
            [uuidv4(), req.user.id, req.params.id, JSON.stringify({ username: user.username, org_id: req.orgId })]
        );
...
router.get('/audit', async (req, res) => {
    try {
       ...
       LIMIT ? OFFSET ?`,
            [req.orgId, Math.min(parseInt(limit) || 50, 200), Math.max(parseInt(offset) || 0, 0)]
        );
        res.json({ logs });
    } catch (err) {
    ...
    try {
        const approvals = await db.all(
            `SELECT pa.*, 
             ...
             LEFT JOIN users req_u ON req_u.id = pa.reviewed_by
             WHERE pa.org_id = ?
...
router.post('/approvals/:id/approve', async (req, res) => {
    try {
        ...
        const approval = await db.get(
            'SELECT * FROM pending_role_approvals WHERE id = ? AND org_id = ? AND status = ?',
            [req.params.id, req.orgId, 'pending']
        );
...
// ─── Governance: KPI Overview — CRCE KPI Tree Engine ─────────────────
// Company Risk Control Effectiveness (0-100)
// T1: Risk Exposure Health (30%) | T2: SLA & Escalation (30%)
// T3: Operational Throughput (25%) | T4: Investigation Quality (15%)
router.get('/governance/kpi-overview', async (req, res) => {
    try {
        ...
        const openSerious = parseInt(openCritical?.open_serious) || 0;
        ...
        const t1_den = Math.max(0, 100 - fraudPer1k * 2);
        const tier1 = Math.round(t1_exp * 0.35 + t1_con * 0.2 + t1_trd * 0.25 + t1_den * 0.2);

        // ═══ TIER 2: SLA & ESCALATION ═══
        const openCount = parseInt(slaSummary?.open) || 0;
        ...
        const t3_bal = loadVar > 0 ? Math.max(0, 100 - loadVar * 3) : 100;
        const tier3 = Math.round(t3_res * 0.35 + t3_bk * 0.25 + t3_spd * 0.25 + t3_bal * 0.15);

        // ═══ TIER 4: QUALITY (PARTIAL) ═══
        const resolvedCnt = parseInt(resolvedTotal?.total) || 0;
        const t4_spd = avgResH > 0 && avgResH < 72 ? 85 : avgResH < 120 ? 65 : 40;
        const t4_con = loadVar < 5 ? 90 : loadVar < 15 ? 70 : 50;
        ...
        const productList = (riskByProduct || []).map(p => {
            const fl = parseInt(p.flagged) || 0;
            return { name: p.name, flagged: fl, exposure: fl * UNIT_VALUE };
        });
...
// POST /api/org/appoint-admin — Org Owner appoints Company Admin by email
        ...
        if (!user) {
            ...
            await db.run(
                `INSERT INTO users (id, username, email, password_hash, role, org_id, user_type, must_change_password, created_at)
                 VALUES (?, ?, ?, ?, 'company_admin', ?, 'org', 1, NOW())`,
                [userId, displayName, email, passwordHash, orgId]
            );
            ...
            await db.run('UPDATE users SET role = ?, org_id = ? WHERE id = ?', ['company_admin', orgId, user.id]);
        }
...
router.post('/owner/appoint', requireOrgOwner(), async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            await db.run(
                `INSERT INTO users (id, username, email, password_hash, role, org_id, user_type, must_change_password, created_at)
                 VALUES ($1, $2, $3, $4, $5, $6, 'org', true, NOW())`,
