...
router.get('/trust-score', cacheMiddleware(120), async (req, res) => {
    try {
        ...
        const creds = await db
            .get('SELECT COUNT(*) as cnt FROM verifiable_credentials WHERE status = ?', ['active'])
            .catch(() => ({ cnt: 0 }));
        res.json(
            reputation.calculateTrustScore({
                product_authenticity: 75,
                supply_chain_transparency: 62,
                esg_performance: 68,
                carbon_integrity: 72,
                compliance_readiness: 65,
                partner_reliability: 70,
                verified_credentials: creds.cnt,
...
router.get('/transparency', cacheMiddleware(120), async (req, res) => {
    try {
        ...
        res.json(
            reputation.calculateTransparencyIndex({
                total_products: products.cnt,
                tracked_products: Math.floor(products.cnt * 0.8),
                total_partners: partners.cnt,
                verified_partners: Math.floor(partners.cnt * 0.6),
                total_shipments: 100,
                traced_shipments: 75,
                has_did: dids.cnt > 0,
                has_vc: true,
                has_blockchain: true,
                scope_3_covered: true,
                gri_reported: true,
            })
        );
    } catch (err) {
        res.status(500).json({ error: 'Transparency index failed' });
    }
});
...
router.get('/carbon-integrity', cacheMiddleware(120), async (req, res) => {
    try {
        ...
        const avgConf =
            credits.length > 0 ? credits.reduce((s, c) => s + (c.mrv_confidence || 0), 0) / credits.length : 0;
        const retiredPct =
            credits.length > 0 ? (credits.filter(c => c.status === 'retired').length / credits.length) * 100 : 0;
        res.json(
            reputation.calculateCarbonIntegrity({
                total_credits: credits.length,
                verified_credits: credits.length,
                avg_mrv_confidence: avgConf,
                double_count_incidents: 0,
                avg_additionality_pass_rate: 85,
                blockchain_anchored_pct: 100,
                retired_pct: retiredPct,
                third_party_verified: false,
            })
        );
    } catch (err) {
        res.status(500).json({ error: 'Carbon integrity failed' });
    }
});
...
router.get('/platform-index', cacheMiddleware(300), async (req, res) => {
    try {
        res.json(reputation.buildPlatformIndex([]));
    } catch (err) {
        res.status(500).json({ error: 'Platform index failed' });
    }
});
...
router.get('/dashboard', cacheMiddleware(120), async (req, res) => {
    try {
        const ts = reputation.calculateTrustScore({
            product_authenticity: 75,
            supply_chain_transparency: 62,
            esg_performance: 68,
            carbon_integrity: 72,
        ...
        const ti = reputation.calculateTransparencyIndex({
            total_products: 50,
            tracked_products: 40,
            total_partners: 20,
        ...
        const ci = reputation.calculateCarbonIntegrity({
            avg_mrv_confidence: 78,
            avg_additionality_pass_rate: 85,
            blockchain_anchored_pct: 100,
            retired_pct: 20,
        });
