...
router.get("/my/profile", authMiddleware, async function(req, res) {
    try {
        var result = await db.all(
            "SELECT * FROM supplier_profiles WHERE org_id = $1 LIMIT 1",
            [req.user.org_id]
        );
...
router.put("/my/profile", authMiddleware, async function(req, res) {
    try {
        var b = req.body;
        var slug = (b.public_name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
        var result = await db.all(
            ...
            [req.user.org_id, b.public_name, slug, b.description, b.website, b.country, JSON.stringify(b.certifications || []), b.logo_url, b.is_published || false]
        );
...
router.get("/my/improvements", authMiddleware, async function(req, res) {
    try {
        var result = await db.all(
            "SELECT improvement_plan FROM supplier_profiles WHERE org_id = $1 LIMIT 1",
            [req.user.org_id]
        );
        ...
        var suggestions = [
            { area: "Certifications", action: "Add ISO 27001, SOC2, or industry-specific certifications", impact: "high" },
...
router.put("/my/assessment", authMiddleware, async function(req, res) {
    try {
        await db.all(
            "UPDATE supplier_profiles SET self_assessment = $1, updated_at = NOW() WHERE org_id = $2",
            [JSON.stringify(req.body.assessment || {}), req.user.org_id]
        );
