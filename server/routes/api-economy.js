...
router.get('/keys', async (req, res) => {
    try {
        const keys = await db.prepare('SELECT api_key,tier,integration_type,app_name,status,rate_limit_rpm,daily_limit,total_requests,created_at FROM api_keys WHERE org_id = ?').all(req.user?.org_id || 'default');
...
router.get('/usage', async (req, res) => {
    try {
        const keys = await db.prepare('SELECT * FROM api_keys WHERE org_id = ?').all(req.user?.org_id || 'default');
