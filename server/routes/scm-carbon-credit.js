...
const ccme = require('../engines/intelligence/carbon-credit-engine');
...
router.post('/pipeline', requirePermission('esg:manage'), async (req, res) => {
    try {
        ...
        const historicalRoutes = await db
            .prepare('SELECT * FROM carbon_simulations WHERE simulated_by = ? ORDER BY created_at DESC LIMIT 12')
            .all(req.user?.id || 'system');

        const result = ccme.runFullPipeline(rawEvent, {
            historicalRoutes: historicalRoutes.map(r => ({
                shipment_id: r.event_id,
                product_id: r.simulation_id,
        ...
        if (result.pipeline === 'minted' && result.credit) {
            ...
            `,
                [
                    ...
                    c.project.name,
                    c.project.type,
                    c.project.intervention,
                    c.project.route_type,
                    c.project.origin_region,
                    layerData.L1_ingestion?.event_id,
                    c.additionality.reduction_uid,
                    c.mrv.confidence_score,
                    c.mrv.verification_hash,
                    c.blockchain.evidence_hash,
                    c.issuer_id,
                    c.org_id,
                    c.current_owner_id,
                    c.beneficiary_id,
...
router.get('/registry', cacheMiddleware(30), async (req, res) => {
    try {
        ...
        if (vintage) {
            conds.push('vintage_year = ?');
            params.push(parseInt(vintage));
        }
...
router.get('/registry/:creditId', async (req, res) => {
    try {
        const orgId = req.orgId || req.user?.org_id || req.user?.orgId || null;
        const credit = orgId
            ? await db.get('SELECT * FROM carbon_credits WHERE credit_id = ? AND org_id = ?', [
                  req.params.creditId,
                  orgId,
              ])
        ...
        const sim = credit.simulation_id
            ? await db.get('SELECT * FROM carbon_simulations WHERE event_id = ? OR simulation_id = ?', [
        ...
        res.status(500).json({ error: 'Credit detail failed' });
    }
});
...
router.post('/transfer', requirePermission('esg:manage'), async (req, res) => {
    try {
        ...
        const result = ccme.transferCredit(credit, new_owner_id, { id: req.user?.id, org_id: req.user?.org_id }, [
        ...
        await db
            .prepare(
                `UPDATE carbon_credits SET status = ?, current_owner_id = ?, provenance = ?, updated_at = NOW() WHERE credit_id = ?`
            )
        ...
        `,
            [
                ...
                req.user?.id,
                credit_id,
                JSON.stringify({ from: credit.current_owner_id, to: new_owner_id, sod_eyes: result.sod?.actual_eyes }),
            ]
        );
...
router.post('/retire', requirePermission('esg:manage'), async (req, res) => {
    try {
        ...
        await db
            .prepare(
                `UPDATE carbon_credits SET status = 'retired', retirement_data = ?, provenance = ?, updated_at = NOW() WHERE credit_id = ?`
            )
        ...
        `,
            [
                ...
                req.user?.id,
                credit_id,
                JSON.stringify({ reason: result.credit.retirement.reason, hash: result.credit.retirement.hash }),
            ]
        );

        res.json({ message: 'Credit retired permanently', ...result });
    } catch (err) {
        res.status(500).json({ error: 'Retirement failed' });
    }
});
...
router.get('/risk-score', cacheMiddleware(60), async (req, res) => {
    try {
        const credits = (await db.all('SELECT * FROM carbon_credits')).map(c => ({
            ...c,
            provenance: JSON.parse(c.provenance || '[]'),
        }));
...
router.get('/verify/:creditId', async (req, res) => {
    try {
        const c = await db.get(
            'SELECT credit_id, serial_number, status, quantity_tCO2e, vintage_year, project_name, project_type, intervention, mrv_confidence, evidence_hash, blockchain_status, created_at FROM carbon_credits WHERE credit_id = ?',
            [req.params.creditId]
        );
        ...
        res.json({
            ...
            mrv_confidence: c.mrv_confidence,
            blockchain: { evidence_hash: c.evidence_hash, status: c.blockchain_status },
            registry: 'TrustChecker CCME v3.0',
            methodology: 'GHG Protocol + DEFRA 2025 + UNFCCC CDM + ISO 14064',
...
router.get('/market-stats', cacheMiddleware(60), async (req, res) => {
    try {
        ...
        credits.forEach(c => {
            const t = c.intervention || 'unknown';
        ...
        res.json({
            ...
            avg_mrv_confidence:
                credits.length > 0
                    ? Math.round(credits.reduce((s, c) => s + (c.mrv_confidence || 0), 0) / credits.length)
                    : 0,
            by_vintage: vintages,
            ...
            by_status: {
                ...
                transferred: credits.filter(c => c.status === 'transferred').length,
                retired: credits.filter(c => c.status === 'retired').length,
            },
            reference_price: { usd_per_tCO2e: 45, source: 'EU ETS reference' },
            settlement_options: ['internal_offset', 'b2b_transfer', 'api_trading'],
        });
    } catch (err) {
        res.status(500).json({ error: 'Market stats failed' });
    }
});
...
router.get('/evidence/:creditId', requirePermission('compliance:view'), async (req, res) => {
    try {
        ...
        const mockMRV = {
            confidence_score: c.mrv_confidence,
            verification_hash: c.mrv_hash || c.evidence_hash,
            checks: { passed: c.mrv_confidence >= 70 ? 10 : 7, total: 11, details: [] },
            methodology: 'GHG Protocol + DEFRA 2025 + ISO 14064',
        };
        const mockAdd = {
            reduction_uid: c.reduction_uid,
            status: 'passed',
            rules: { passed: 6, total: 6, details: [] },
        ...
        res.status(500).json({ error: 'Evidence package generation failed' });
    }
});
...
router.get('/simulations', cacheMiddleware(30), async (req, res) => {
    try {
        ...
        const sims = orgId ? await db.prepare(q).all(orgId) : await db.prepare(q).all();
        res.json({ title: 'MRV Simulation History', total: sims.length, simulations: sims });
    } catch (err) {
        res.status(500).json({ error: 'Simulations query failed' });
    }
});
