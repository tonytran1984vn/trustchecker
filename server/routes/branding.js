...
router.get('/', async (req, res) => {
    try {
        const brandConfig = await db.get(
            "SELECT details FROM audit_log WHERE action = 'BRAND_CONFIG' AND actor_id = ? ORDER BY timestamp DESC LIMIT 1",
            [req.user.company || req.user.id]
        );
    ...
    try {
        ...
        for (const field of colorFields) {
            if (theme[field] && !/^#[0-9A-Fa-f]{6}$/.test(theme[field])) {
                return res
                    .status(400)
                    .json({ error: `Invalid color format for ${field}: ${theme[field]}. Use #RRGGBB.` });
            }
        }
