            ...
            templates: [
                {
                    ...
                    description: 'Product scan volume, results, fraud scores',
                    sections: ['scans', 'results', 'fraud_scores'],
                },
                {
                    id: 'fraud-report',
                    name: 'Fraud Detection Report',
                    description: 'Fraud alerts by severity, status, product',
                    sections: ['alerts', 'severity', 'products'],
                },
                {
                    id: 'product-report',
                    name: 'Product Trust Report',
                    description: 'Product trust scores, compliance, QR activity',
                    sections: ['trust', 'compliance', 'qr'],
                },
                {
                    id: 'compliance-report',
                    name: 'Compliance & GDPR Report',
                    description: 'GDPR exports, deletions, retention policies',
                    sections: ['gdpr', 'retention', 'audit'],
                },
                {
                    id: 'supply-chain-report',
                    name: 'Supply Chain Report',
                    description: 'Partner performance, events, leak analysis',
                    sections: ['partners', 'events', 'leaks'],
                },
                {
                    id: 'financial-report',
                    name: 'Financial Report',
                    description: 'Revenue, invoices, plan distribution',
                    sections: ['revenue', 'invoices', 'plans'],
                },
            ],
            formats: ['json', 'csv'],
            total: 6,
    ...
    try {
     ...
     LIMIT 1000`,
        ...
        const summary = {
            ...
            avg_fraud_score:
                scans.length > 0
                    ? Math.round((scans.reduce((a, s) => a + (s.fraud_score || 0), 0) / scans.length) * 1000) / 1000
                    : 0,
            avg_trust_score:
                scans.length > 0 ? Math.round(scans.reduce((a, s) => a + (s.trust_score || 0), 0) / scans.length) : 0,
            avg_response_ms:
                scans.length > 0
                    ? Math.round(scans.reduce((a, s) => a + (s.response_time_ms || 0), 0) / scans.length)
                    : 0,
        };
...
router.get('/product-report', requirePermission('report:view'), async (req, res) => {
    ...
    `,
        ...
        const summary = {
            ...
            avg_trust_score:
                products.length > 0
                    ? Math.round(products.reduce((a, p) => a + (p.trust_score || 0), 0) / products.length)
                    : 0,
            trust_distribution: {
...
router.get('/compliance-report', requirePermission('report:export'), async (req, res) => {
    try {
        ...
        const gdprDeletions =
            (await db.get("SELECT COUNT(*) as c FROM audit_log WHERE action = 'GDPR_DELETION'"))?.c || 0;
        ...
        const complianceRecords = await db.all(
            'SELECT framework, status, COUNT(*) as count FROM compliance_records GROUP BY framework, status'
        );
...
router.get('/supply-chain-report', requirePermission('report:view'), async (req, res) => {
    try {
        ...
        const partners = await db.all(
            'SELECT * FROM partners' + orgFilter + ' ORDER BY created_at DESC LIMIT 1000',
            orgP
        );
        const events = await db.all(
            'SELECT event_type, COUNT(*) as count FROM supply_chain_events' +
                orgFilter +
                ' GROUP BY event_type ORDER BY count DESC LIMIT 1000',
            orgP
        );
        ...
        try {
            leaks = await db.all(
                'SELECT status, COUNT(*) as count FROM fraud_alerts' + orgFilter + ' GROUP BY status',
                orgP
            );
        } catch (e) {
            /* table may not exist */
        ...
        const summary = {
            ...
            avg_partner_trust:
                partners.length > 0
                    ? Math.round(partners.reduce((a, p) => a + (p.trust_score || 0), 0) / partners.length)
                    : 0,
...
router.get('/financial-report', requirePermission('report:export'), async (req, res) => {
    try {
        ...
        const plans = await db.all(
            "SELECT plan_name, COUNT(*) as count FROM billing_plans WHERE status = 'active' GROUP BY plan_name"
        );
...
router.get('/export/:entity', requirePermission('report:export'), async (req, res) => {
    try {
        ...
        const entityMap = {
            users: {
                sql: 'SELECT id, username, email, role, company, status, created_at FROM users',
                filename: 'users',
            },
            ...
            fraud_alerts: {
                sql: 'SELECT * FROM fraud_alerts ORDER BY created_at DESC LIMIT 1000',
                filename: 'fraud_alerts',
            },
            evidence: {
                sql: 'SELECT id, title, sha256_hash, status, tags, created_at FROM evidence_items',
                filename: 'evidence',
            },
            ...
            audit: {
                sql: 'SELECT id, actor_id, action, entity_type, entity_id, timestamp FROM audit_log ORDER BY timestamp DESC LIMIT 1000',
                filename: 'audit_log',
            },
        };
