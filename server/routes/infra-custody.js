...
router.get('/security', cacheMiddleware(120), async (req, res) => {
    try {
        res.json(custodyEngine.assessSecurityPosture());
    } catch (err) {
...
router.get('/hash-chain', cacheMiddleware(300), async (req, res) => {
    try {
        res.json(custodyEngine.verifyHashChain([]));
    } catch (err) {
        res.status(500).json({ error: 'Hash chain verification failed' });
    }
});
...
router.get('/keys', requirePermission('admin:manage'), async (req, res) => {
    try {
        res.json(custodyEngine.getKeyManagementStatus());
    } catch (err) {
        res.status(500).json({ error: 'Key status failed' });
    }
});
...
router.get('/disaster-recovery', cacheMiddleware(300), async (req, res) => {
    try {
        res.json(custodyEngine.checkDisasterRecovery());
    } catch (err) {
        res.status(500).json({ error: 'DR check failed' });
    }
});
...
router.get('/separation-of-powers', async (req, res) => {
    try {
        res.json(custodyEngine.verifySeparationOfPowers());
    } catch (err) {
        res.status(500).json({ error: 'SoP verification failed' });
    }
});

// GET /boundary — IT role boundary
router.get('/boundary', (req, res) => {
    res.json({ title: 'IT Role Boundary (Cryptographic Custodian)', ...custodyEngine.getITBoundary() });
});
