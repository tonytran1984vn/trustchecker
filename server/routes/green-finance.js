...
router.get('/collateral', cacheMiddleware(120), async (req, res) => {
    try {
        ...
        try {
            credits = orgId
                ? await db.all(`SELECT * FROM carbon_credits WHERE org_id = ? AND status IN ('active','minted')`, [orgId])
        ...
        res.json({
            title: 'Carbon Credit Collateral',
            total_tCO2e: 0, total_credits: 0, spot_price_usd: 25,
            haircut_pct: 30, collateral_value_usd: 0, max_borrowing_usd: 0
        });
    }
});
...
router.get('/dashboard', cacheMiddleware(120), async (req, res) => {
    try {
        ...
        let creditCount = 0, totalValue = 0;
        try {
            const stats = orgId
                ? await db.get(`SELECT COUNT(*) as cnt, COALESCE(SUM(quantity_tCO2e),0) as total FROM carbon_credits WHERE org_id = ?`, [orgId])
        ...
        res.json({ total_credits: 0, total_tCO2e: 0, estimated_value_usd: 0, active_instruments: 0, avg_green_score: 0 });
    }
});
