...
router.post(
    ...
    validate({ body: schemas.createPurchaseOrder }),
    async (req, res) => {
        try {
            const orgId = getOrgId(req);
...
router.post('/purchase-orders/:id/approve', requirePermission('po:approve'), async (req, res) => {
    try {
        const orgId = getOrgId(req);
        const po = await db.get('SELECT id, created_by FROM purchase_orders WHERE id = ? AND org_id = ?', [
            req.params.id,
            orgId,
        ]);
        ...
        await db
            .prepare(
                'UPDATE purchase_orders SET status = ?, approved_by = ?, approved_at = NOW(), updated_at = NOW() WHERE id = ? AND org_id = ?'
            )
...
router.put('/data/incidents/:id', validate({ body: schemas.updateIncident }), async (req, res) => {
    try {
        ...
        if (status && !validStatuses.includes(status)) {
            return res
                .status(400)
                .json({ error: `Invalid status: ${status}. Must be one of: ${validStatuses.join(', ')}` });
        }
        ...
        if (status === 'resolved' || status === 'closed') {
            ...
            try {
                ...
                console.debug('[TrustRecalc] Skip:', e.message);
            }
        }
...
router.get('/supplier-scoring', async (req, res) => {
    try {
        ...
        const locations = await db.all(
            'SELECT * FROM partner_locations WHERE status = ? ORDER BY created_at LIMIT 1000',
            ['active']
        );
...
router.post(
    '/suppliers/onboard',
    requirePermission('supplier:onboard'),
    validate({ body: schemas.onboardSupplier }),
    async (req, res) => {
        try {
            ...
            const existing = await db.get(
                'SELECT id, name, normalized_name, tier FROM partners WHERE normalized_name = ?',
                [normalized]
            );
            if (existing) {
...
router.get('/suppliers/:id', async (req, res) => {
    try {
        ...
        const locations = await db.all(
            'SELECT * FROM partner_locations WHERE partner_id = ? ORDER BY created_at LIMIT 1000',
            [req.params.id]
        );
...
router.get('/geo-alerts', async (req, res) => {
    try {
        ...
        const params = [];
        if (orgId) {
            sql += ' AND (p.org_id = ? OR p.org_id IS NULL)';
            params.push(orgId);
        }
...
router.get('/mismatch-alerts', async (req, res) => {
    try {
        ...
        const params = [];
        if (orgId) {
            sql += ' AND (ad.org_id = ? OR ad.org_id IS NULL)';
            params.push(orgId);
        }
        ...
        if (productIds.size > 0) {
            ...
            try {
                ...
                const products = await db.all(
                    `SELECT id, name FROM products WHERE id IN (${placeholders}) LIMIT 1000`,
                    [...productIds]
                );
...
router.get('/receiving', async (req, res) => {
    try {
        ...
        const params = [];
        if (orgId) {
            sql += ' AND (fp.org_id = ? OR tp.org_id = ?)';
