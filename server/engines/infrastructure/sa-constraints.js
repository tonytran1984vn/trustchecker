...
const RESTRICTED_ACTIONS = {
    credit_edit: {
        ...
        daily_limit: 3,
        description: 'Edit credit quantity/status directly',
    },
    credit_delete: {
        severity: 'critical',
        requires: 'compliance_officer',
        daily_limit: 0,
        description: 'Delete a carbon credit (forbidden)',
    },
    ...
    risk_override: {
        ...
        requires: 'risk_lead',
        daily_limit: 5,
        description: 'Override risk engine decision',
    },
    baseline_change: {
        severity: 'high',
        requires: 'compliance_officer',
        daily_limit: 3,
        description: 'Change carbon baseline factor',
    },
...
const FORBIDDEN_ACTIONS = [
    ...
    {
        action: 'Modify deployed model weights',
        reason: 'Model weights are frozen — new version required via governance',
    },
...
class SuperAdminConstraints {
    ...
    checkAction(action, saUserId) {
        ...
        if (restriction.daily_limit === 0) {
            return {
                allowed: false,
                reason: `Action "${action}" is permanently forbidden for SA`,
                action,
                type: 'forbidden',
            };
        }
        ...
        if (count >= restriction.daily_limit) {
            return {
                allowed: false,
                reason: `Daily limit exceeded (${count}/${restriction.daily_limit})`,
                action,
                type: 'rate_limited',
            };
        }
    ...
    requestApproval(params) {
        ...
        const request = {
            ...
            hash: crypto
                .createHash('sha256')
                .update(JSON.stringify({ requestId, action, sa_user_id, reason }))
                .digest('hex'),
    ...
    processApproval(requestId, approver) {
        ...
        // 1. Time-Of-Check to Time-Of-Use Validation
        if (new Date() > new Date(request.expires_at)) {
            request.status = 'expired';
            return { error: 'Request expired. Dual-approvals must complete within 24 hours.' };
        }

        // 2. Dual-Key Role Matching
        if (role !== request.required_approver_role) {
            return {
                error: 'Approver does not possess the required constitutional role: ' + request.required_approver_role,
            };
        }
        ...
        return {
            ...
            message: decision === 'approve' ? 'Action approved and logged' : 'Action rejected and logged',
        };
    }
    ...
    getConstraintDashboard() {
        ...
        const todayActions = Object.entries(this._dailyCounts)
            ...
            .map(([k, v]) => ({
                action: k.split(':').slice(1).join(':'),
                count: v,
                limit: RESTRICTED_ACTIONS[k.split(':').slice(1).join(':')]?.daily_limit || '∞',
            }));
