...
class NetworkIntelligenceEngine {
    ...
    async getSupplierIntelligence(supplierName) {
        const result = await db.all('SELECT * FROM supplier_network_intelligence WHERE supplier_name ILIKE $1', [
            supplierName,
        ]);
    ...
    async searchSuppliers(query, options) {
        ...
        const minOrgs = options.min_orgs || 2;
    ...
    async refreshIntelligence() {
        ...
        const count = await db.all('SELECT count(*) FROM supplier_network_intelligence');
        await db.all(
