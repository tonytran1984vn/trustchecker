...
// FIX BUG-01: Bounded Sets to prevent unbounded memory growth (OOM)
// ═══════════════════════════════════════════════════════════════════════════════

const MAX_DEDUP_KEYS = 50000; // Cap at 50K entries (~4MB), FIFO eviction
...
/** Bounded Set.add — evicts oldest entries when capacity exceeded */
function _boundedSetAdd(set, value) {
    set.add(value);
    if (set.size > MAX_DEDUP_KEYS) {
        // FIFO eviction: delete first (oldest) entries until under cap
        const excess = set.size - MAX_DEDUP_KEYS;
        let removed = 0;
...
class CarbonCreditMintingEngine {
    ...
    ingestEvent(rawEvent) {
        ...
        _boundedSetAdd(_idempotencyKeys, idempotencyKey);
        ...
        const event = {
            ...
            warehouse_type:
                warehouse_type ||
                (product_category === 'Healthcare' || product_category === 'F&B' ? 'cold_storage' : 'ambient'),
            storage_days,
            origin_region,
            timestamp,
            fingerprint: {
    ...
    computeBaseline(event, historicalData = null) {
        ...
        const baselines = [
            ...
            {
                type: 'regulatory',
                emission: regulatoryEmission,
                source: `${regBL.standard} (×${regBL.multiplier})`,
                mode: staticBL.mode,
            },
        ];
        ...
        const conservative = baselines.filter(b => b.type !== 'historical').sort((a, b) => a.emission - b.emission)[0];

        // If historical is lower, use historical (even more conservative)
        const selected =
            historicalEmission !== null && historicalEmission < conservative.emission
    ...
    verifyMRV(event, counterfactual, partnerData = null) {
        ...
        checks.push({
            layer: 'measurement',
            check: 'Route distance plausibility',
            passed: distanceOK,
            detail: `${event.distance_km} km`,
            deduct: 15,
        });
        ...
        checks.push({
            layer: 'measurement',
            check: 'Emission calculation integrity',
            passed: calcOK,
            detail: `Reduction: ${counterfactual.reduction.transport_kgCO2e} kgCO₂e`,
            deduct: 20,
        });
        ...
        checks.push({
            layer: 'measurement',
            check: 'Transport mode mapping',
            passed: modeOK,
            detail: event.actual_mode,
            deduct: 10,
        });
        ...
        checks.push({
            layer: 'reporting',
            check: 'GHG Protocol field compliance',
            passed: ghgOK,
            detail: ghgOK ? 'All required fields present' : 'Missing product_id or shipment_id',
            deduct: 10,
        });
        ...
        checks.push({
            layer: 'reporting',
            check: 'ISO 14064 methodology reference',
            passed: true,
            detail: 'DEFRA 2025 + GHG Protocol',
            deduct: 0,
        });
        ...
        checks.push({
            layer: 'verification',
            check: 'Device fingerprint validated',
            passed: fpOK,
            detail: fpOK ? 'Fingerprint confirmed' : 'No fingerprint (lower confidence)',
            deduct: 10,
        });
        ...
        if (partnerData) {
            ...
            checks.push({
                layer: 'verification',
                check: 'Partner trust threshold (≥40)',
                passed: partnerOK,
                detail: `Trust: ${partnerData.trust_score || 50}`,
                deduct: 10,
            });
        ...
        checks.push({
            layer: 'verification',
            check: 'Time consistency (≤90 days)',
            passed: timeOK,
            detail: timeOK ? 'Within window' : 'Outside 90-day window',
            deduct: 10,
        });
        ...
        checks.push({
            layer: 'verification',
            check: 'Reduction plausibility (<95%)',
            passed: pctOK,
            detail: `${reductionPct}% reduction`,
            deduct: 15,
        });
        ...
        const mrvStatus =
            confidenceScore >= 70 ? 'verified' : confidenceScore >= 40 ? 'conditionally_verified' : 'failed';
    ...
    checkAdditionality(event, counterfactual, historicalRoutes = []) {
        ...
        rules.push({
            rule: 'Mode shift required',
            passed: modeDiff,
            detail: modeDiff ? `${event.baseline_mode} → ${event.actual_mode}` : 'Same mode — no shift',
            severity: 'critical',
        });
        if (!modeDiff) passed = false;

        // Rule 2: Not seasonal fluctuation (check historical routes)
        const historicalSame = historicalRoutes.filter(
            r => r.actual_mode === event.actual_mode && r.route_type === event.route_type
        );
        const isSeasonalRepeat = historicalSame.length >= 6; // If same mode used ≥6 of last 12 months
        rules.push({
            rule: 'Not seasonal fluctuation',
            passed: !isSeasonalRepeat,
            detail: isSeasonalRepeat
                ? `Same mode used ${historicalSame.length}/12 months — likely BAU`
                : `Mode used ${historicalSame.length}/12 months — valid shift`,
            severity: 'high',
        });
        if (isSeasonalRepeat) passed = false;

        // Rule 3: Not a repeat claim (no similar shipment claimed before)
        const isDuplicate = historicalRoutes.some(
            r => r.shipment_id === event.shipment_id && r.product_id === event.product_id
        );
        rules.push({
            rule: 'No duplicate claim',
            passed: !isDuplicate,
            detail: isDuplicate ? 'Similar shipment already claimed' : 'No prior claim found',
            severity: 'critical',
        });
        ...
        rules.push({
            rule: 'Minimum threshold (≥100 kg)',
            passed: meetsMin,
            detail: `${counterfactual.reduction.transport_kgCO2e} kgCO₂e`,
            severity: 'medium',
        });
        ...
        rules.push({
            rule: 'Not suspicious (≤95% reduction)',
            passed: notSuspicious,
            detail: `${counterfactual.reduction.percentage}%`,
            severity: 'high',
        });
        ...
        rules.push({
            rule: 'Unique Reduction ID (anti-double-count)',
            passed: !isDoubleCounted,
            detail: isDoubleCounted ? 'DUPLICATE: reduction already registered' : `UID: ${reductionUID.slice(0, 16)}…`,
            severity: 'critical',
        });
        if (isDoubleCounted) passed = false;

        // Register UID if passing (bounded to prevent OOM)
        if (passed && !isDoubleCounted) {
            _boundedSetAdd(_reductionUIDs, reductionUID);
        }
    ...
    mintCredit(params) {
        ...
        if (!additionalityResult || !additionalityResult.passed) {
            return {
                error: 'Additionality check failed — cannot mint',
                additionality_status: additionalityResult?.status,
            };
        }
        ...
        const evidencePayload = {
            credit_id: creditId,
            serial: serialNumber,
            reduction_kgCO2e: reductionKg,
            credit_tonnes: creditTonnes,
    ...
    transferCredit(credit, newOwnerId, transferredBy, approvals = []) {
        ...
        // FIX BUG-08: Require explicit org_id to prevent SoD bypass on cross-org transfers
        if (!transferredBy || !transferredBy.org_id) {
            return { error: 'transferredBy.org_id is required for transfer audit trail' };
        }

        // SoD: check 4-eyes for standard, 6-eyes for cross-org
        const isCrossOrg = credit.org_id && credit.org_id !== transferredBy.org_id;
            ...
            credit: {
                ...
                provenance: [
                    ...(credit.provenance || []),
                    {
                        ...
                        from: credit.current_owner_id,
                        to: newOwnerId,
                        by: transferredBy.id || transferredBy,
                        at: timestamp,
                        hash: transferHash,
                        sod_eyes: sodReq.eyes,
                    },
                ],
            },
            sod: sodCheck,
    ...
    generateEvidencePackage(credit, mrvResult, additionalityResult, counterfactual) {
        return {
            ...
            legal_notice:
                'This evidence package is generated by TrustChecker Carbon Credit Minting Engine v3.0. All data is cryptographically sealed and blockchain-anchored for tamper-proof verification. Suitable for regulatory submission, court proceedings, and audit verification under GHG Protocol, ISO 14064, and UNFCCC CDM standards.',
        };
    }
    ...
    assessCreditRisk(credit, partnerData = null, historicalData = null) {
        ...
        factors.push({
            factor: 'Partner Trust',
            weight: '30%',
            value: partnerTrust,
            risk_contribution: Math.round(partnerRisk * 10) / 10,
            detail: `Trust: ${partnerTrust}/100`,
        });
        riskScore += partnerRisk;

        // Factor 2: Route anomaly (25%)
        const routeAnomaly = credit.mrv?.confidence_score
            ? Math.max(0, ((100 - credit.mrv.confidence_score) / 100) * 25)
            : 12.5;
        factors.push({
            factor: 'Route Anomaly',
            weight: '25%',
            value: credit.mrv?.confidence_score || 50,
            risk_contribution: Math.round(routeAnomaly * 10) / 10,
            detail: `MRV confidence: ${credit.mrv?.confidence_score || 50}`,
        });
        ...
        factors.push({
            ...
            weight: '15%',
            value: hasFingerprint ? 0 : 100,
            risk_contribution: deviceRisk,
            detail: hasFingerprint ? 'Fingerprint validated' : 'No device fingerprint',
        });
        ...
        factors.push({
            factor: 'Baseline Volatility',
            weight: '20%',
            value: reductionPct * 5,
            risk_contribution: Math.round(baselineRisk * 10) / 10,
            detail: credit.project?.intervention || 'Unknown intervention',
        });
        ...
        factors.push({
            factor: 'Historical Drift',
            weight: '10%',
            value: 0,
            risk_contribution: histDrift,
    ...
    assessPortfolioRisk(credits = [], simulations = []) {
        ...
        if (dupUIDs.length > 0) {
            signals.push({
                ...
                severity: 'critical',
                detail: `${dupUIDs.length} duplicate reduction UID(s)`,
                impact: 30,
            });
            totalRisk += 30;
        }
        ...
        if (outliers.length > 0) {
            signals.push({
                type: 'unusual_volume',
                severity: 'high',
                detail: `${outliers.length} credits exceed 3× avg (${Math.round(avgTonnes * 1000) / 1000} tCO₂e)`,
                impact: 15,
            });
            totalRisk += 15;
        }
        ...
        if (recent.length > 10) {
            signals.push({
                type: 'rapid_issuance',
                severity: 'medium',
                detail: `${recent.length} credits in 24h`,
                impact: 10,
            });
            totalRisk += 10;
        }
        ...
        if (lowConf.length > 0) {
            signals.push({
                type: 'low_mrv_confidence',
                severity: 'high',
                detail: `${lowConf.length} credits with MRV confidence < 70`,
                impact: 15,
            });
            totalRisk += 15;
        }
        ...
        if (suspicious.length > 0) {
            signals.push({
                ...
                severity: 'critical',
                detail: `${suspicious.length} simulations with >90% reduction`,
                impact: 25,
            });
            totalRisk += 25;
        }
        ...
        return {
            ...
            integrity: {
                ...
                additionality_validated: true,
            },
            recommendation:
                totalRisk >= 50
                    ? 'HALT issuance — investigate'
                    : totalRisk >= 25
                      ? 'Review flagged credits'
                      : 'All clear',
            assessed_at: new Date().toISOString(),
        };
    }
    ...
    computeCarbonIndex(orgData = []) {
        const index = orgData
            .map(t => {
                ...
                const avgConfidence =
                    t.credits?.length > 0
                        ? t.credits.reduce((s, c) => s + (c.mrv?.confidence_score || 50), 0) / t.credits.length
                        : 0;
                const retiredPct =
                    t.credits?.length > 0
                        ? (t.credits.filter(c => c.status === 'retired').length / t.credits.length) * 100
                        : 0;
        ...
        return {
            ...
            total_platform_reduction_tCO2e:
                Math.round(index.reduce((s, t) => s + t.total_reduction_tCO2e, 0) * 1000) / 1000,
            index,
            generated_at: new Date().toISOString(),
        };
    }
    ...
    runFullPipeline(rawEvent, options = {}) {
        ...
        const mintResult = this.mintCredit({
            ...
            counterfactual,
            mrvResult: mrv,
            additionalityResult: additionality,
            issuer_id: rawEvent.issuer_id || 'system',
            org_id: rawEvent.org_id,
            beneficiary_id: rawEvent.beneficiary_id,
            fractional_policy,
        });
        ...
        return {
            ...
            layers: {
                ...
                L2_baseline: {
                    status: 'ok',
                    type: baseline.selected.type,
                    emission: baseline.selected.emission_kgCO2e,
                },
    ...
    _creditId(vintage, mode) {
        ...
        const seq = Math.floor(Math.random() * 99999999)
            .toString()
            .padStart(8, '0');
    ...
    getSoDRequirements() {
        return SOD_REQUIREMENTS;
    }
    getBaselineDefaults() {
        return BASELINE_DEFAULTS;
    }
    getRegulatoryBaselines() {
        return REGULATORY_BASELINES;
    }
}
