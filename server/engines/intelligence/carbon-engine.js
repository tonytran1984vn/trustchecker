...
const CONFIDENCE_LEVELS = [
    ...
    { level: 3, label: 'Supplier-reported', description: 'Supplier-provided primary data', color: '#3b82f6' },
    {
        ...
        description: 'Published sector emission factors (DEFRA/IEA)',
        color: '#f59e0b',
    },
...
const REGULATORY_FRAMEWORKS = [
    {
        ...
        region: 'EU',
        scopes_required: [1, 2, 3],
        data_type: 'Per-product',
        status: 'active',
    ...
    },
    {
        id: 'sec_climate',
        name: 'SEC Climate',
        full: 'SEC Climate Disclosure Rules',
        region: 'US',
        scopes_required: [1, 2],
        data_type: 'Corporate',
        status: 'phased',
    ...
    },
    {
        id: 'iso_14064',
        name: 'ISO 14064',
        full: 'GHG Quantification & Reporting',
        region: 'Global',
        scopes_required: [1, 2, 3],
        data_type: 'Corporate',
        status: 'voluntary',
    },
    {
        ...
        full: 'Paris Climate Accord Targets',
        region: 'Global',
        scopes_required: [1, 2, 3],
        data_type: 'National',
        status: 'active',
        targets: { 2030: 0.55, 2050: 0.1 },
    },
    {
        id: 'vn_green',
        name: 'Vietnam Green Growth',
        full: 'Vietnam National Green Growth Strategy',
        region: 'Vietnam',
        scopes_required: [1, 2],
        data_type: 'National',
        status: 'active',
...
const MATURITY_LEVELS = [
    {
        ...
        description: 'Per-product footprint calculation',
        requirements: ['scope_calculation'],
        target: 'Baseline',
    },
    {
        level: 2,
        name: 'ESG Governance Module',
        description: 'GRI reporting + offset + blockchain',
        requirements: ['gri_reporting', 'offset_recording', 'blockchain_anchor'],
        target: 'Enterprise Ready',
    },
    {
        level: 3,
        name: 'Carbon Intelligence',
        description: 'Risk integration + cross-org benchmarks',
        requirements: ['risk_integration', 'cross_org_benchmark', 'partner_esg_scoring'],
        target: 'Regulated Industry',
    },
    {
        level: 4,
        name: 'Industry Carbon Index',
        description: "Moody's-style ESG rating platform",
        requirements: ['industry_index', 'real_time_monitoring', 'compliance_api'],
        target: 'Cross-border',
    },
    {
        level: 5,
        name: 'Carbon Trading Platform',
        description: 'Offset marketplace + verification network',
        requirements: ['offset_marketplace', 'verification_network', 'carbon_credit_trading'],
...
class CarbonEngine {
    ...
    calculateFootprint(product, shipments = [], events = [], partner = null) {
        ...
        const scope1 = {
            ...
            disclosure:
                'Category-based industry-average cradle-to-gate emission intensity estimate. May not reflect facility-specific combustion data.',
        };
        ...
        const scope2 = {
            ...
            disclosure:
                'Estimated using modeled warehouse emission factors. Where actual electricity consumption data is available, both location-based and market-based approaches may be calculated.',
        };
        ...
        const scope3 = {
            ...
            disclosure:
                'Scope 3 Categories 4 (Upstream Transportation) & 9 (Downstream Transportation) screening-level estimates based on shipment distance, mode, and product weight.',
        };
        ...
        return {
            ...
            methodology_note:
                'Screening-level estimation for decision-support. Not a substitute for ISO 14064 certified inventory.',
            eas_version: '3.0',
    ...
    aggregateByScope(products, shipments, events) {
        ...
        const avgIntensity =
            productFootprints.length > 0
                ? Math.round(
                      (productFootprints.reduce((s, p) => s + (p.intensity || 0), 0) / productFootprints.length) * 100
                  ) / 100
                : 0;
        const avgConfidence =
            productFootprints.length > 0 ? Math.round((totalConfidence / productFootprints.length) * 10) / 10 : 0;

        // Build per-scope breakdown items from category accumulators
        const buildItems = (byCat, scopeTotal) =>
        ...
        const highConfidenceCount =
            (confidenceCounts[3] || 0) + (confidenceCounts[4] || 0) + (confidenceCounts[5] || 0);
        const highConfidenceRatio =
            productFootprints.length > 0 ? Math.round((highConfidenceCount / productFootprints.length) * 100) : 0;

        return {
            total_emissions_kgCO2e: Math.round(total * 100) / 100,
            total_emissions_tonnes: Math.round((total / 1000) * 100) / 100,
            scope_1: {
                total: Math.round(scope1Total * 100) / 100,
                pct: total > 0 ? Math.round((scope1Total / total) * 100) : 0,
                label: 'Scope 1 — Modeled Production Estimate',
                items: buildItems(s1ByCat, scope1Total),
            },
            scope_2: {
                total: Math.round(scope2Total * 100) / 100,
                pct: total > 0 ? Math.round((scope2Total / total) * 100) : 0,
                label: 'Scope 2 — Modeled Energy & Warehousing',
                items: buildItems(s2ByCat, scope2Total),
            },
            scope_3: {
                total: Math.round(scope3Total * 100) / 100,
                pct: total > 0 ? Math.round((scope3Total / total) * 100) : 0,
                label: 'Scope 3 — Transport & Distribution (Cat 4 & 9)',
                items: buildItems(s3ByCat, scope3Total),
            },
    ...
    partnerLeaderboard(partners, shipments, violations) {
        return partners
            .map(p => {
                ...
                const lateCount = partnerShipments.filter(
                    s =>
                        s.actual_delivery &&
                ...
                const reliabilityWeight =
                    partnerShipments.length > 0 ? (1 - lateCount / partnerShipments.length) * 30 : 15;
                    ...
                    metrics: {
                        ...
                        shipment_reliability:
                            partnerShipments.length > 0
                                ? Math.round((1 - lateCount / partnerShipments.length) * 100) + '%'
                                : 'N/A',
    ...
    generateGRIReport(data) {
        ...
        return {
            ...
            report_note:
                'This report supports GRI 305-1/2/3/5 disclosure mapping. It is not a substitute for externally assured sustainability reports.',
            reporting_period: {
            ...
            disclosures: {
                'GRI 305-1': {
                    ...
                    value: scopeData?.scope_1?.total || 0,
                    unit: 'kgCO2e',
                },
                'GRI 305-2': {
                    title: 'Energy Indirect GHG Emissions (Scope 2)',
                    value: scopeData?.scope_2?.total || 0,
                    unit: 'kgCO2e',
                },
                'GRI 305-3': {
                    title: 'Other Indirect GHG Emissions (Scope 3)',
                    value: scopeData?.scope_3?.total || 0,
                    unit: 'kgCO2e',
                },
                'GRI 305-5': {
                    title: 'Reduction of GHG Emissions',
                    value: scopeData?.reduction_targets?.paris_aligned_2030 || 0,
                    unit: 'kgCO2e',
                    note: '2030 Paris-aligned target (45% reduction)',
                },
                'GRI 308-1': {
                    title: 'Supplier Environmental Assessment',
                    value: leaderboard?.length || 0,
                    unit: 'suppliers assessed',
                },
                'GRI 414-1': {
                    title: 'Supplier Social Assessment',
                    value: leaderboard?.filter(p => p.grade === 'A' || p.grade === 'B').length || 0,
                    unit: 'suppliers passing',
                },
            },
    ...
    calculateRiskFactors(scopeData, leaderboard) {
        ...
        const avgIntensity =
            scopeData?.avg_intensity_kgCO2e_per_unit || total / Math.max(1, scopeData?.products_assessed || 1);
        ...
        return {
            ...
            description:
                'Carbon intensity integrated into operational and supply chain risk scoring, enabling proactive mitigation before regulatory or market escalation.',
    ...
    assessRegulatory(scopeData, leaderboard, offsets) {
        return REGULATORY_FRAMEWORKS.map(reg => {
            ...
            for (const scope of reg.scopes_required) {
                ...
                checks.push({
                    check: `Scope ${scope} data`,
                    status: hasData ? 'pass' : 'warn',
                    detail: hasData ? `${scopeData?.[key]?.total || 0} kgCO2e` : 'No data yet',
                });
            ...
            if (reg.id === 'eu_csrd') {
                checks.push({
                    check: 'GRI 305 disclosures',
                    status: 'pass',
                    detail: 'Auto-generated by Carbon Engine',
                });
                checks.push({
                    check: 'Supplier assessment (GRI 308/414)',
                    status: (leaderboard?.length || 0) > 0 ? 'pass' : 'warn',
                    detail: `${leaderboard?.length || 0} partners assessed`,
                });
            }
            ...
            if (reg.id === 'paris' || reg.id === 'eu_cbam') {
                ...
                checks.push({
                    check: 'Carbon offset records',
                    status: hasOffsets ? 'pass' : 'info',
                    detail: hasOffsets ? `${offsets} offsets blockchain-anchored` : 'Optional',
                });
            }

            // Blockchain proof
            checks.push({
                check: 'Tamper-proof evidence',
                status: 'pass',
                detail: 'Blockchain seal + SHA-256 hash chain',
            });
    ...
    assessMaturity(features) {
        ...
        return {
            ...
            recommendation:
                currentLevel < 5
                    ? MATURITY_LEVELS[currentLevel]?.description || 'Continue development'
    ...
    getRoleMatrix() {
        return {
            ...
            actions: [
                'view_passport',
                'view_scope_breakdown',
                'view_transport_detail',
                'view_esg_leaderboard',
                'submit_offset',
                'configure_factors',
                'export_gri_report',
                'cross_org_benchmark',
            ...
            matrix: {
                scm_ops: {
                    view_passport: true,
                    view_scope_breakdown: true,
                    view_transport_detail: true,
                    view_esg_leaderboard: false,
                    submit_offset: false,
                    configure_factors: false,
                    export_gri_report: false,
                    cross_org_benchmark: false,
                    view_esg_kpi: false,
                },
                risk: {
                    view_passport: true,
                ...
                compliance: {
                    view_passport: true,
                    view_scope_breakdown: true,
                    view_transport_detail: 'read_only',
                    view_esg_leaderboard: true,
                    submit_offset: false,
                    configure_factors: false,
                    export_gri_report: true,
                    cross_org_benchmark: false,
                    view_esg_kpi: false,
                },
                company_admin: {
                ...
                super_admin: {
                    ...
                    configure_factors: true,
                    export_gri_report: true,
                    cross_org_benchmark: true,
                    view_esg_kpi: false,
                },
                ceo: {
                    view_passport: 'aggregated',
                    view_scope_breakdown: false,
                    view_transport_detail: false,
                    view_esg_leaderboard: 'top_5',
                    submit_offset: false,
                    configure_factors: false,
                    export_gri_report: false,
                    cross_org_benchmark: false,
    ...
    getGovernanceFlow() {
        return {
            ...
            flow: [
                {
                    step: 1,
                    layer: 'Layer 5 — Integration',
                    name: 'SCM Data Collection',
                    components: ['Supply Routes', 'Shipment Tracking', 'Partner Management', 'Warehouse Events'],
                    icon: '📦',
                },
                {
                    step: 2,
                    layer: 'Layer 3 — Intelligence',
                    name: 'Carbon Engine Calculation',
                    components: ['Scope 1 (Manufacturing)', 'Scope 2 (Warehousing)', 'Scope 3 (Transport)'],
                    icon: '⚡',
                },
                {
                    step: 3,
                    ...
                    components: ['ESG → Risk Factor', 'Grade D/F → Supply Risk', 'Partner C/D → Distributor Risk'],
                    icon: '🎯',
                },
                {
                    step: 4,
                    ...
                    components: ['GRI 305-1/2/3/5', 'EU CSRD / CBAM', 'SEC Climate Rules'],
                    icon: '⚖️',
                },
                {
                    step: 5,
                    layer: 'Layer 4 — Integrity',
                    name: 'Blockchain Seal',
                    components: ['SHA-256 Hash', 'Evidence Store', 'Public Verification Portal'],
                    icon: '🔗',
                },
                {
                    step: 6,
                    ...
                    name: 'CEO Dashboard',
                    components: ['ESG Grade KPI', '2030 Target', 'Offset Proof', 'Board Narrative'],
                    icon: '👔',
                },
            ],
            principle:
                'Carbon sits between: Data → Risk → Governance → Executive. Governance amplifier — not a standalone module.',
        };
    }
    ...
    assessScope3Materiality(products = [], shipments = [], events = [], partners = []) {
        ...
        const categories = [
            ...
            },
            {
                ...
                recommendation:
                    totalShipments > 0
                        ? 'Already calculated from shipment data — highest confidence'
                        : 'Add shipment records for accurate calculation',
            },
            {
                id: 5,
            ...
            },
            {
                ...
                priority: products.some(p => ['Electronics', 'IoT', 'Sensor'].includes(p.category))
                    ? 'material'
                    : 'immaterial',
                screening_score: products.some(p => ['Electronics', 'IoT', 'Sensor'].includes(p.category)) ? 55 : 10,
                recommendation:
                    'Material for electronics (energy-using products) — use power consumption × grid factor',
            },
            {
                id: 12,
        ...
        return {
            ...
            guidance:
                'Focus on material categories first. Immaterial categories can be reported with proxy estimates. "Needs Data" categories require additional data integration.',
        };
    }
    ...
    calculateNetEmissions(scopeData, offsets = []) {
        ...
        const activeOffsets = offsets.filter(
            o => o.status === 'retired' || o.status === 'active' || o.status === 'minted'
        );
    ...
    verifyOffset(offset) {
        ...
        if (vintage && currentYear - vintage > 5) {
            issues.push({
                severity: 'warning',
                message: `Vintage ${vintage} is ${currentYear - vintage} years old (max recommended: 5 years)`,
            });
        }
        ...
        if (registry && !validRegistries.some(r => registry.includes(r))) {
            issues.push({
                severity: 'warning',
                message: `Registry "${offset.registry || offset.provider}" not in recognized list (Verra, Gold Standard, ACR, CAR)`,
            });
        }
        ...
        if (offset.status === 'retired') {
            issues.push({
                severity: 'error',
                message: 'Credit already retired — cannot retire again (double-counting)',
            });
        }
            ...
            categories: [
                {
                    id: 1,
                    name: 'Purchased Goods & Services',
                    kgCO2e: Math.round(cat1Total * 100) / 100,
                    ...
                    items: cat1Items.slice(0, 20),
                    item_count: cat1Items.length,
                },
                {
                    id: 4,
                    name: 'Upstream Transportation',
                    kgCO2e: Math.round(cat4Total * 100) / 100,
                    ...
                    items: cat4Items.slice(0, 20),
                    item_count: cat4Items.length,
                },
                {
                    id: 9,
                    name: 'Downstream Transportation',
                ...
                },
                {
                    id: 11,
                    name: 'Use of Sold Products',
                    kgCO2e: Math.round(cat11Total * 100) / 100,
                    ...
                    items: cat11Items.slice(0, 20),
                    item_count: cat11Items.length,
                },
            ],
            totals: {
                cat1: Math.round(cat1Total),
                cat4: Math.round(cat4Total),
                cat9: cat9Total,
                cat11: Math.round(cat11Total),
            },
        };
    }
        ...
        return {
            ...
            disclosures: {
                'E1-1': {
                    ...
                    status: total > 0 ? 'In Progress' : 'Not Started',
                ...
                'E1-6': {
                    ...
                    scope_1_kgCO2e: s1,
                    scope_2_kgCO2e: s2,
                    scope_3_kgCO2e: s3 || scope3Deep?.total_scope3_kgCO2e || 0,
                    total_kgCO2e: total,
                    total_tCO2e: Math.round(total / 10) / 100,
                },
                'E1-7': {
            ...
            cbam_annex: {
                ...
                avg_embedded_kgCO2e:
                    scopeData?.products_assessed > 0
                        ? Math.round((total / scopeData.products_assessed) * 100) / 100
                        : 0,
            },
            supplier_assessment: {
                'S2-1': {
        ...
        for (const [code, d] of Object.entries(disclosures)) {
            const val =
                d.value !== undefined
                    ...
                    : d.total_kgCO2e !== undefined
                      ? `${d.total_kgCO2e.toLocaleString()} kgCO₂e`
...
<hr style="margin-top:40px"><p style="color:#64748b;font-size:0.75rem">Generated by TrustChecker Carbon Intelligence Engine v4.0 · ${r.standard || r.report_standard || 'GHG Protocol'}</p>
    ...
    calculateCrossOrgBenchmark(orgScopeData, allOrgsSummaries = []) {
        ...
        const allIntensities = allOrgsSummaries
            .map(o => {
    ...
    evaluateMarketplaceCredit(credit) {
        ...
        if (vintageAge > 5) {
            risk_score += 30;
            risk_factors.push(`Old vintage (${vintage}, ${vintageAge}y ago)`);
        } else if (vintageAge > 3) {
            risk_score += 15;
            risk_factors.push(`Aging vintage (${vintage})`);
        }
        ...
        if (!validRegistries.some(r => registry.includes(r))) {
            risk_score += 25;
            risk_factors.push(`Unrecognized registry: ${credit.registry || 'Unknown'}`);
        ...
        const PROJECT_PRICES = {
            ...
            reforestation: 15,
            afforestation: 14,
            avoided_deforestation: 18,
            'redd+': 20,
            cookstove: 6,
            methane_capture: 12,
            direct_air_capture: 150,
            blue_carbon: 25,
            biochar: 30,
            unknown: 10,
        };
        ...
        return {
            ...
            risk_color:
                risk_score <= 25 ? '#10b981' : risk_score <= 50 ? '#f59e0b' : risk_score <= 75 ? '#ef4444' : '#991b1b',
            risk_factors,
            ...
            quality_assessment: {
                vintage_score:
                    vintageAge <= 1 ? 'excellent' : vintageAge <= 3 ? 'good' : vintageAge <= 5 ? 'acceptable' : 'poor',
                registry_tier: premiumRegistries.some(r => registry.includes(r))
                    ? 'premium'
                    : validRegistries.some(r => registry.includes(r))
                      ? 'standard'
                      : 'unverified',
    ...
    calculateIntensity(totalFootprint, product) {
        ...
        if (physicalIntensity <= benchmark.p20) {
            ...
            percentile =
                20 + Math.round(((physicalIntensity - benchmark.p20) / (benchmark.median - benchmark.p20)) * 30);
        } else if (physicalIntensity <= benchmark.p80) {
            percentile =
                50 + Math.round(((physicalIntensity - benchmark.median) / (benchmark.p80 - benchmark.median)) * 30);
        } else {
        ...
        return {
            ...
            vs_industry:
                physicalIntensity <= benchmark.p20
                    ? 'top_performer'
                    : physicalIntensity <= benchmark.median
                      ? 'above_average'
                      : physicalIntensity <= benchmark.p80
                        ? 'below_average'
                        : 'critical',
        };
    }
    ...
    // ─── Internal Helpers ────────────────────────────────────────────────────
    // FIX BUG-10: Multi-tenant geo — accept origin from shipment, not hardcoded HCMC
    _estimateDistance(shipment) {
        if (shipment.current_lat && shipment.current_lng) {
            const R = 6371;
            // Use shipment origin if available, otherwise fall back to org default or HCMC
            const originLat = shipment.from_lat || shipment.origin_lat || 10.8;
            const originLng = shipment.from_lng || shipment.origin_lng || 106.6;
            const lat1 = (originLat * Math.PI) / 180;
            const lat2 = (shipment.current_lat * Math.PI) / 180;
            const dLat = lat2 - lat1;
            const dLon = ((shipment.current_lng - originLng) * Math.PI) / 180;
    ...
    _gradeByIntensity(intensity, category) {
        ...
        if (intensity <= benchmark.p20) {
            return {
                grade: 'A',
                label: `Top performer (≤${benchmark.p20} ${benchmark.unit})`,
                color: '#10b981',
                percentile: 'top_20',
            };
            ...
            return {
                grade: 'B',
                label: `Above average (industry median: ${benchmark.median})`,
                color: '#3b82f6',
                percentile: '20_40',
            };
            ...
            return {
                grade: 'B',
                label: `Near median (industry median: ${benchmark.median})`,
                color: '#3b82f6',
                percentile: '40_50',
            };
            ...
            return {
                grade: 'F',
                label: `Critical — restructure supply chain`,
                color: '#991b1b',
    ...
    _overallESGGrade(scopeData, leaderboard) {
        ...
        const avgPartnerScore =
            leaderboard.length > 0 ? leaderboard.reduce((s, p) => s + p.esg_score, 0) / leaderboard.length : 50;
