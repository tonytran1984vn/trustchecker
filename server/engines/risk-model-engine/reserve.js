...
class RiskReserveEngine {
    ...
    contribute(totalTxRevenue, breakdown = {}) {
        ...
        for (const [reserveId, policy] of Object.entries(RESERVE_POLICY)) {
            ...
            contributions[reserveId] = {
                amount: this._round(amount),
                new_balance: this._round(this.balances[reserveId]),
            };
    ...
    fileClaim(reserveId, amount, reason, claimant, evidence = null) {
        ...
        if (amount > maxClaim) {
            return {
                error: `Claim $${amount} exceeds max ${policy.max_claim_pct}% ($${this._round(maxClaim)}) of ${policy.name}`,
            };
        }

        // Determine approval tier
        let approver = 'auto';
        const autoApprove = amount <= CHARGEBACK_PROTOCOL.auto_approve_below;
        if (!autoApprove) {
        ...
        const claim = {
            ...
            approver,
            investigation_deadline: new Date(
                Date.now() + CHARGEBACK_PROTOCOL.investigation_sla_hours * 3600000
            ).toISOString(),
            resolution_deadline: new Date(
                Date.now() + CHARGEBACK_PROTOCOL.max_resolution_days * 86400000
            ).toISOString(),
    ...
    getReserveHealth() {
        ...
        return {
            ...
            claims_summary: {
                ...
                total_paid_out: this._round(
                    this.claims.filter(c => c.status === 'paid_out').reduce((a, c) => a + c.amount, 0)
                ),
            },
        };
    }
    ...
    getChargebackProtocol() {
        return CHARGEBACK_PROTOCOL;
    }
    getClaims(limit = 20) {
        return this.claims.slice(-limit).reverse();
    }
    getContributions(limit = 20) {
        return this.contributions.slice(-limit).reverse();
    }
