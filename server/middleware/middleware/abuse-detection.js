...
function checkAbuse(type) {
    return (req, res, next) => {
        ...
        if (count > threshold) {
            ...
            // S-09: Log abuse event to DB for persistence across restarts
            try {
                const db = require('../db');
                db.run(
                    "INSERT INTO audit_log (id, action, entity_type, entity_id, metadata, created_at, org_id) VALUES ($1, $2, $3, $4, $5, NOW(), $6)",
                    [require('crypto').randomUUID(), 'ABUSE_DETECTED', 'rate_limit', orgId, JSON.stringify({ type, count, threshold }), orgId]
                ).catch(() => {});
            } catch(e) {}
